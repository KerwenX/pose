...
class prop_rot_loss(nn.Module):
    ...
    def prop_point_matching_loss(self, points, p_g_vec, f_g_vec, p_r_vec, f_r_vec, p_t, g_R, g_t, sym):
        ...
        # only for non-symmetric objects
        # handle c1, c2
        points_re = torch.bmm(g_R.permute(0, 2, 1), (points - g_t.view(bs, 1, -1)).permute(0, 2, 1).float())
        points_re = points_re.permute(0, 2, 1)
        ...
        points_re_n = torch.matmul(p_R.transpose(-2,-1), (points-p_t.unsqueeze(-2)).transpose(-2,-1).float()).transpose(-2,-1)  # bs x 1024, 3
    ...
    def get_y_reflection_pc_b(self, PC, p_t, p_g_vec, PC_re, sym):
        ...
        vec_along_p_g = torch.matmul(torch.matmul(pc_t_res.float(), p_g_vec.unsqueeze(-1).float()).float(),
                                           p_g_vec.unsqueeze(-2).float())  # bs x 1028 x 3
        a_to_1_2_b = vec_along_p_g - pc_t_res
    ...
    def get_yx_reflection_pc_b(self, p_r_vec, p_g_vec, PC, PC_re, sym, p_t):
        ...
        t = -(torch.matmul(p_z.unsqueeze(-2), PC.transpose(-2, -1).float())
              - batch_dot(p_z, p_t).view(-1, 1, 1).float())  # 1 x  1028
    ...
    def prop_sym_matching_loss(self, PC, PC_re, p_g_vec, p_r_vec, p_t, gt_R, gt_t, sym):
        ...
        bs = PC.shape[0]
        points_re_cano = torch.bmm(gt_R.permute(0, 2, 1), (PC - gt_t.view(bs, 1, -1)).permute(0, 2, 1).float())
