...
def defor_3D_bb_in_batch(pc, model_point, R, t, s, sym=None, aug_bb=None):
    pc_reproj = torch.matmul(R.transpose(-1, -2), (pc - t.unsqueeze(-2)).transpose(-1, -2).float()).transpose(-1, -2)
    ...
    pc_new = (torch.matmul(R, pc_reproj.transpose(-2, -1)) + t.unsqueeze(-1).float()).transpose(-2, -1)
    s_new = s * new_aug_bb
...
def defor_3D_rt_in_batch(pc, R, t, aug_rt_t, aug_rt_r):
    ...
    t_new = t + aug_rt_t
    pc_new = torch.matmul(aug_rt_r, pc_new.transpose(-2,-1).float()).transpose(-2,-1)
