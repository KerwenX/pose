...
from network.fs_net_repo.Unet import ImageFeat
...
class PoseNet9D(nn.Module):
    ...
    def forward(self, points, obj_id, roi_img, roi_mask):
        ...
        # recon = points
        # img_feat = self.image_feat(roi_img)
        mask_feat = self.mask_feat(roi_mask)
        feat = (mask_feat+2*feat)/3
        # if FLAGS.train:
        ...
        return p_green_R, p_red_R, f_green_R, f_red_R, Pred_T, Pred_s
